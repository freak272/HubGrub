...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const { businessCode, isDefaultBusiness, setBusinessCode } = useBusiness();
  ...
  const { data: bizProfile } = useQuery<BizProfile>({
    queryKey: ["biz-profile-sidebar", businessCode],
  ...
  return (
    <div className="flex min-h-screen w-full bg-background flex-col md:flex-row">
      <aside
        className="w-full md:w-64 border-r flex-shrink-0 flex flex-col"
        ...
        <div className="p-5 border-b" style={{ borderColor: "hsl(var(--sidebar-border))" }}>
          <Link href="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
            <div
              ...
              style={{ backgroundColor: role === "customer" ? theme.primaryHex : "hsl(var(--primary))" }}
            >
        ...
        {role && (
          <div className="px-4 pt-4">
            <div
              ...
              style={{ backgroundColor: role === "customer" ? theme.badgeBg : "hsl(var(--sidebar-accent) / 0.4)", border: `1px solid ${role === "customer" ? theme.borderColor : "hsl(var(--sidebar-border))"}` }}
            >
              <div
                className="h-9 w-9 rounded-full flex items-center justify-center shrink-0 text-sm"
                style={{ backgroundColor: role === "customer" ? theme.bgLight : "hsl(var(--muted))" }}
              >
              ...
              <div className="min-w-0 flex-1">
                ...
                {role === "business" && (
                  <div className="text-xs mt-0.5" style={{ color: "hsl(var(--sidebar-foreground) / 0.5)" }}>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-md transition-colors hover:opacity-90"
        ...
        <div className="p-4 border-t space-y-3" style={{ borderColor: "hsl(var(--sidebar-border))" }}>
          {role && (
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 rounded-md text-xs transition-colors hover:bg-destructive/10 hover:text-destructive"
